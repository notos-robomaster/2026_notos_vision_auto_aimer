...
: debug_(debug), detector_(config_path, false)
{
  ...
  // 预分配输入缓冲区
  input_buf_ = cv::Mat(640, 640, CV_8UC3, cv::Scalar(0, 0, 0));
}
...
std::list<Armor> YOLO11::detect(const cv::Mat & raw_img, int frame_count)
{
  ...
  cv::Mat bgr_img;
  if (debug_) tmp_img_ = raw_img;  // 仅调试时拷贝
  if (use_roi_) {
  ...
  // preproces（复用预分配缓冲区，仅清零有效区域外的填充区域）
  input_buf_.setTo(cv::Scalar(0, 0, 0));
  auto roi = cv::Rect(0, 0, w, h);
  cv::resize(bgr_img, input_buf_(roi), {w, h});
  ov::Tensor input_tensor(ov::element::u8, {1, 640, 640, 3}, input_buf_.data);

  /// infer（复用 infer_request_，避免每帧创建开销）
