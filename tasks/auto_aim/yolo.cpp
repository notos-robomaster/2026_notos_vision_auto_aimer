...
#include "yolos/yolo11_opencv.hpp"
#else
...
YOLO::YOLO(const std::string & config_path, bool debug)
{
  ...
  tools::logger()->info("YOLO model: {}", yolo_name);

#ifdef USE_OPENCV_DNN
  // Jetson/ARM 平台：使用 OpenCV DNN
  tools::logger()->info("Platform: ARM/Jetson, using OpenCV DNN backend");
  if (yolo_name == "yolo11" || yolo_name == "yolov8" || yolo_name == "yolov5") {
    yolo_ = std::make_unique<YOLO11OpenCV>(config_path, debug);
  } else {
    tools::logger()->error("Unknown model: {}, defaulting to yolo11_opencv", yolo_name);
    yolo_ = std::make_unique<YOLO11OpenCV>(config_path, debug);
  }
#else
  // x86 平台：使用 OpenVINO
  tools::logger()->info("Platform: x86, using OpenVINO backend");
  if (yolo_name == "yolov8") {
