...
int main(int argc, char * argv[])
{
  ...
  auto config_path = cli.get<std::string>("config-path");
  auto display = cli.get<int>("display") != 0;
  ...
  try {
    ...
    constexpr double FILTER_ALPHA = 0.5;  // 滤波系数，提高以减少延迟
    constexpr double DEADZONE_DEG = 0.03;  // 死区阈值（度），减小以提高响应速度
    
    // 第二层：移动平均滤波器（平滑高速运动）
    constexpr int MA_WINDOW_SIZE = 3;  // 移动平均窗口大小（从5降为3，降低延迟）
    std::deque<double> pitch_history;  // pitch历史队列
    std::deque<double> yaw_history;    // yaw历史队列
    
    // 第三层：自适应滤波系数（根据目标速度调整）
    constexpr double MIN_ALPHA = 0.4;   // 低速时的滤波系数（提高，减少延迟）
    constexpr double MAX_ALPHA = 0.85;  // 高速时的滤波系数（提高，更快响应）
    constexpr double SPEED_THRESHOLD_LOW = 0.5;   // 低速阈值 (m/s)，降低以更早切换
    constexpr double SPEED_THRESHOLD_HIGH = 2.5;  // 高速阈值 (m/s)
    ...
    while (!exiter.exit()) {
      ...
      if (fps_elapsed >= 1000) {  // 每秒更新一次FPS
        ...
        tools::logger()->info("FPS: {:.1f}", current_fps);
        fps_frame_count = 0;
      ...
      if (simple_state && simple_state->valid) {
        ...
        // 调试输出：接收到的原始姿态（每300帧输出一次，降低IO开销）
        if (display && frame_count % 300 == 0) {
...
// #endif
        ...
        } else if (plan.control && current_frame_count % 30 == 0) {
          // 控制台输出（每30帧一次）
          std::cout << fmt::format(
...
// #endif
        ...
        } else if (command.control && current_frame_count % 30 == 0) {
          // 控制台输出（每30帧一次）
          std::cout << fmt::format(
      ...
      // 在图像上绘制调试信息（仅在显示模式下绘制，避免无用的图像操作）
      if (display) {
      ...
      // 仅在显示模式下绘制GUI（避免 imshow/waitKey 阻塞主循环）
      if (display) {
